			...
			function init() {
				...
				for ( i = 0; i < xgrid; i ++ ) {
					...
					for ( j = 0; j < ygrid; j ++ ) {
						...
						geometry = new THREE.SphereGeometry( xsize, ysize, xsize );
						...
						material.color.setHSL( material.hue, material.saturation, 3 );
						...
						mesh.dx = 0.001 * ( 0.9 - Math.random() );
						mesh.dy = 0.001 * ( 0.9 - Math.random() );
			...
			function render() {
				...
				for ( let i = 0; i < cube_count; i ++ ) {
					...
					material.color.setHSL( h, material.saturation, 0.9 );

				}

				if ( counter % 100000 > 200 ) {
					...
					for ( let i = 0; i < cube_count; i ++ ) {
						...
						mesh.position.x -= 300 * mesh.dx;
						mesh.position.y += 300 * mesh.dy;
						mesh.position.z += 700 * mesh.dx;
